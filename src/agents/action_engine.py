...
if __name__ == "__main__":
    ...
    print(f"\nTesting merchant: {merchant_id}")

    try:

        # Import decision agent here to avoid circular imports
        from src.agents.decision_agent import decide_action

        decision_result = decide_action(merchant_id)
        ...
        if not isinstance(decision_result, dict):
            ...
            print("ERROR: Decision agent did not return a dictionary.")
            print(f"Received: {decision_result}")

        else:

            print(
                f"Merchant:   "
                f"{decision_result.get('merchant_id', merchant_id)}"
            )

            print(
                f"Risk Score: "
                f"{decision_result.get('risk_score', 'N/A')}"
            )

            print(
                f"Decision:   "
                f"{decision_result.get('decision', 'N/A')}"
            )

            print(
                f"Reason:     "
                f"{decision_result.get('reason', 'N/A')}"
            )
