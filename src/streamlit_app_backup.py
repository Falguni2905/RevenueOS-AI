...
with operations_tab:
    ...
    st.caption(
        "Monitor investigation cases, prioritize review queues, "
        "and complete human approvals."
    )
    ...
    if not cases:
        ...
        if selected_risk != "All":
            filtered_cases = [
                ...
                if str(case.get("risk_category", "")).upper()
                == selected_risk.upper()
            ]
        ...
        if selected_action != "All":
            filtered_cases = [
                case
                for case in filtered_cases
                if str(case.get("recommended_action", "")).upper()
        ...
        st.subheader("Priority Merchant Risk Queue")
        st.caption(
            "Higher priority indicates greater risk and/or immediate "
        ...
        for case in filtered_cases:
            table_data.append(
                {
                    "Merchant": case.get("merchant_id", "N/A"),
                    "Risk Score": round(
                        safe_float(case.get("risk_score", 0)),
                        2,
                    ),
